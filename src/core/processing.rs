...
use crate::utils::{broadcast, CONFIG, DTNCORE, is_local_node_id, peer_find_by_remote, peers_cla_for_node, routing_notify, store_get_bundle, store_get_metadata, store_has_item, store_push_bundle, store_remove};
...
pub async fn transmit(mut bp: BundlePack) -> Result<()> {
    ...
    let src = &bp.source;
    if src != &bp7::EndpointID::none() && (*DTNCORE.lock().unwrap()).get_endpoint_mut(src).is_none() {
        info!(
...
pub async fn receive(mut bndl: Bundle) -> Result<()> {
    ...
    for cb in bndl.canonicals.iter() {
        ...
        if flags.contains(BlockControlFlags::BLOCK_STATUS_REPORT) {
            ...
            if (*CONFIG.lock().unwrap()).generate_status_reports {
...
pub async fn dispatch(bp: BundlePack) -> Result<()> {
    ...
    if (*DTNCORE.lock().unwrap()).is_in_endpoints(&bp.destination)
...
async fn handle_previous_node_block(mut bundle: Bundle) -> Result<Bundle> {
    if let Some(pnb) = bundle.extension_block_by_type_mut(bp7::canonical::PREVIOUS_NODE_BLOCK) {
        let prev_eid = &pnb
            ...
            .clone();
        let local_eid = (*CONFIG.lock().unwrap()).host_eid.clone();
        ...
        for n in nodes {
            ...
            let task_handle = tokio::spawn(async move {
                ...
                if n.transfer(&[bd]).await {
                    ...
                    // if (*CONFIG.lock().unwrap()).generate_service_reports {
...
pub async fn local_delivery(mut bp: BundlePack) -> Result<()> {
    ...
    bp.sync()?;
    if let Some(_aa) = (*DTNCORE.lock().unwrap()).get_endpoint_mut(&bp.destination) {
        broadcast(&bndl);
    }
    if is_local_node_id(&bp.destination) {
        if bndl
...
async fn send_status_report(
    ...
    if (*DTNCORE.lock().unwrap()).is_in_endpoints(&bndl.primary.report_to) {
        return;
    }
